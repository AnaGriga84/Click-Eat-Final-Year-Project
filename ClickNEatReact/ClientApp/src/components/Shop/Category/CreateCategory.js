...
function CreateCategory(props)//props ---imutable data--- aria-live="polite" className="text-center " color="danger"
{
    const [category, SetCategory] = useState(""); //states --- like props.....props imutable, states not....allow to change the variables in realtime
    ...
    const handleSubmit = (e) => {
        ...
        if (category.trim().length > 0) {
            CategoryServices.insertCategory({ name: category }).then(function (resp)
            {
                console.log(resp);
    ...
    return ( // the tags that is going to be rendered on the screen
